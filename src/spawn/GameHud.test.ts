...
describe("mountGameHud", () => {
  ...
  it("the mouse-only bank-open button and the K key both toggle the bank overlay", () => {
    ...
    bankButton?.click();
    expect(document.querySelectorAll(".lw-inv-overlay")[1]?.hasAttribute("hidden")).toBe(false);
    bankButton?.click();
    expect(document.querySelectorAll(".lw-inv-overlay")[1]?.hasAttribute("hidden")).toBe(true);
  ...
  it("depositing into the bank overlay updates hud.bank and does not disturb the I-key inventory binding", () => {
    ...
    window.dispatchEvent(new KeyboardEvent("keydown", { key: "k", bubbles: true }));
    const bankOverlay = document.querySelectorAll(".lw-inv-overlay")[1] as HTMLElement;
