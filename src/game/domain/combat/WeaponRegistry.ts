...
/** Composition-root singleton (E7.1) — mirrors `CREATURE_REGISTRY`/
 *  `PROJECTILE_REGISTRY`: every item with a `combat` block in
 *  `STARTER_ITEMS` (the single source of truth), indexed once here for the
 *  host-side melee resolution (`SpawnFieldView`) to look up by equipped
 *  item id. */
