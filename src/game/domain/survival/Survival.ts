...
export function spawnSurvival(): SurvivalState {
  return { hunger: HUNGER_MAX, stamina: STAMINA_MAX, staminaGated: false };
}
...
function resolveGate(state: SurvivalState, stamina: number): boolean {
  if (stamina <= 0) return true;
  return state.staminaGated && stamina < STAMINA_MAX * STAMINA_GATE_RECOVER_FRAC;
}
...
export function tickSurvival(
  ...
  const stamina = effectiveSprint
    ...
    : Math.min(STAMINA_MAX, state.stamina + dt * STAMINA_REGEN_PER_S);

  return { hunger, stamina, staminaGated: resolveGate(state, stamina) };
}

/** Spend stamina+hunger for one attack swing; a no-op if attack is gated. */
export function drainStaminaForAttack(state: SurvivalState): SurvivalState {
