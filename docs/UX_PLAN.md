...
| E8.6 Menus, lobby & settings overhaul | `MainMenuView`/`LobbyView`/`SettingsView`/`CreditsScreen`/`LoadingScreen` restyle onto E8.1 chrome; Settings UI category (hud style, tooltip verbosity, colorblind palette, reduce-flair); lobby becomes the "play together" surface | ✅ Done (2026-07-20) — colorblind/reduce-flair/tooltip-verbosity controls + lobby restyle; tooltip-verbosity *consumer* deferred (see below) |
| E8.7 HUD cohesion & action bar | Unify hotbar/vitals/minimap/objective/party/combat-meter into one visual system; togglable ability/consumable action bar; gentle buff/effect strip; toasts restyled | ✅ Done (b5c009a) — ActionBar + BuffStrip mounted in `GameHud.ts` (not TerrainScene); ability-cast + buff-source deferred (see below) |
| E8.8 Accessibility, responsive & colorblind pass | Wire `--lw-*` tokens under `[data-high-contrast="true"]` (closes the recorded gap); ship colorblind rarity palette; full keyboard nav + ARIA for chrome/tooltip/menu; mobile layouts | ✅ Done (2026-07-20, 5a79ae8) — colorblind palette + ContextMenu aria fix + mobile `@media` block |
...
- **E8.7 HUD cohesion & action bar's composition-root wiring — RESOLVED (2026-07-20, b5c009a).**
  `ActionBar.ts`/`BuffStrip.ts` are now mounted in `src/spawn/GameHud.ts` — the game-code HUD
  composition root that already owns the live inventory and appends HUD elements to `body`, so
  TerrainScene never had to be touched (the earlier "off-limits engine dir" framing was avoidable).
  Still deferred (recorded in both domain modules' doc comments): per-ability client-side cooldown
  tracking (E7.3 built host-authoritative cast resolution only — `ActionBarState.buildAbilitySlots`
  accepts a `readyFractions` override ready for one; ability-slot activation is a no-op until then)
  and a real buff/status-effect source (none exists yet — `BuffStrip.render` takes a plain chip array,
  rendered `[]` today, ready for whenever one lands).
- **E8.6 tooltip-verbosity consumer (2026-07-20).** The `tooltipVerbosity: "full" | "compact"` setting
  is validated, persisted, and exposed as a Settings control, but no consumer reads it yet —
  `RichTooltip`/`domain/ui/TooltipModel.ts` still always render the full card. Wiring a compact variant
  means threading the setting through every tooltip call site (inventory/hotbar/action-bar/chest/bank/
  trade); deferred as its own follow-up. `colorblindRarity` and `reduceFlair` both ship WITH live
  consumers — only `tooltipVerbosity` is inert.
